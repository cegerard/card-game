 ...
 * valeurs actuelles (base + XP) plutôt que ses seules valeurs de base
 * (Notion > Calcul du score global > § Puissance de combat).
 *
 ...
 *
 * XP convertible = XP cumulée brute à ce stade ; le plafond par palier de
 * fusion (étape 7 du plan) n'est pas encore appliqué.
 */
export function computeCardPower(
  ...
  return (Object.keys(STAT_WEIGHTS) as (keyof CardStats)[]).reduce(
    (total, stat) => {
      const gain = progression.experience * XP_MULTIPLIERS[stat];
