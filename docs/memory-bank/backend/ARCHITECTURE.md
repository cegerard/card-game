...
└── fight/                            # Fight domain module
    ...
    │   │   │   ├── stance/           # Stance (name + remainingTurns) + StanceActivation
...
- **Status Immunity**: `FightingCard.setState()` returns whether the state was accepted and refuses every status while `isStatusImmune`. The four status effects report nothing when refused. Elemental marks are not status effects and still land.
