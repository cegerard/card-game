...
describe('with only two cards each', () => {
  ...
  describe('when the player1 has the slowest cards in speed weighted card pool selection', () => {
    let player1AttackCount: number;
    let player2AttackCount: number;
    let res: FightResult;
    ...
    beforeAll(() => {
      ...
      const player1 = new Player('Player 1', [card3, card4]);
      const player2 = new Player('Player 2', [card1, card2]);
      const fight = new Fight(
        player1,
        player2,
        new SpeedWeightedCardSelector(player1, player2),
    ...
    it('attack less frequently', () => {
      expect(player2AttackCount).toBeGreaterThanOrEqual(player1AttackCount);
    });
